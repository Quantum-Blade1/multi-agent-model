...
When RBI publishes new circulars, a team member runs `python -m ingestion.run_pipeline` to rebuild the index. No model retraining required -- the decision agent immediately sees the new regulations at query time.
...
python -m ingestion.run_pipeline  # build FAISS index
