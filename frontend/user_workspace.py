...
# Ensure api_client can be imported
...
with tab2:
    ...
    with st.form("compliance_check_form", clear_on_submit=False):
        ...
        c1, c2 = st.columns(2)
        name = c1.text_input("Full Name", value="Rahul Sharma")
...
with tab4:
    st.markdown("### Local Session Audit Trail")
    
    history = st.session_state.get("user_request_history", [])
    if not history:
        st.info("No interactive requests have been submitted in this session. Run a New Compliance Check to populate history.")
    else:
        df = pd.DataFrame(history)
        
        statuses = ["All"] + sorted(df["status"].unique().tolist()) if "status" in df.columns else ["All"]
        filter_status = st.selectbox("Filter by Result", statuses)
