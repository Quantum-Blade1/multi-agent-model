...
from rag_pipeline.chunker.chunker import RegulatoryChunker
from rag_pipeline.config import settings
from rag_pipeline.ingestion.embedder import PipelineEmbedder
from rag_pipeline.ingestion.faiss_builder import FAISSBuilder
from rag_pipeline.scraper.cleaner import TextCleaner
from rag_pipeline.scraper.pdf_extractor import PDFExtractor
from rag_pipeline.scraper.rbi_scraper import scrape_rbi
...
def _write_manifest(stage: str, duration: float, input_count: int, output_count: int, index_type: str):
    ...
    manifest_path = os.path.join("rag_pipeline", "data", "pipeline_manifest.json")
...
async def chunk_stage(force: bool, index_type: str):
    stage = "chunk"
    output_path = os.path.join("rag_pipeline", "data", "chunks.json")
    if os.path.exists(output_path) and not force:
...
async def embed_stage(force: bool, index_type: str):
    stage = "embed"
    chunks_path = os.path.join("rag_pipeline", "data", "chunks.json")
    embeddings_path = os.path.join("rag_pipeline", "data", "embeddings.npy")
    chunks_out_path = os.path.join("rag_pipeline", "data", "chunks_embedded.json")
